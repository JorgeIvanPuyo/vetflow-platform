...
import type { ClinicService, InventoryItem, SaleItem, SaleWritePayload } from "@/types/api";

import { SaleCustomerSelector } from "./sale-customer-selector";
...
export function SaleFormScreen({ saleId }: { saleId?: string }) {
  ...
  useEffect(() => {
    ...
    getSale(saleId).then(({ data }) => {
      ...
      setOwnerId(data.owner_id ?? ""); setOwnerName(data.owner_name_snapshot ?? ""); setPatientId(data.patient_id ?? ""); setPatientName(data.patient_name_snapshot ?? ""); setSaleDate(data.sale_date); setNotes(data.notes ?? "");
  ...
  return <form className="page-stack sales-page" onSubmit={handleSubmit}>
    ...
    <section className="panel sale-form-section"><div className="section-heading"><h2>Cliente y datos</h2><p>El cliente y paciente son opcionales para permitir ventas de mostrador.</p></div><div className="sale-header-grid">
      <SaleCustomerSelector value={{ ownerId, ownerName, patientId, patientName }} onChange={(customer) => {
        setOwnerId(customer.ownerId); setOwnerName(customer.ownerName);
        setPatientId(customer.patientId); setPatientName(customer.patientName);
      }} />
