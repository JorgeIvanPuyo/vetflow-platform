...
export function SaleDetailScreen({ saleId }: { saleId: string }) {
  ...
  return <div className="page-stack sales-page">
    ...
    <section className="inventory-table-card" aria-label="Líneas de venta"><div className="inventory-table-scroll"><table className="inventory-table sale-table"><thead><tr><th>Tipo</th><th>Descripción</th><th>Cantidad</th><th>Unidad</th><th>Precio</th><th>Descuento</th><th>Subtotal</th><th>Total</th></tr></thead><tbody>{sale.items.map((item) => <tr key={item.id} className="inventory-table__row--static"><td><span className="badge">{item.line_type === "product" ? "Producto" : "Servicio"}</span></td><td className="sale-text"><strong>{item.description_snapshot}</strong><small>{item.internal_code_snapshot || "Línea manual"}</small></td><td>{item.quantity}</td><td>{item.unit_snapshot}</td><td className="sale-money">{formatPurchaseCurrency(item.unit_price_ars, moneyPreferences)}</td><td>{item.discount_percentage}% · {formatPurchaseCurrency(item.line_discount_ars, moneyPreferences)}</td><td className="sale-money">{formatPurchaseCurrency(item.line_subtotal_ars, moneyPreferences)}</td><td className="sale-money"><strong>{formatPurchaseCurrency(item.line_total_ars, moneyPreferences)}</strong></td></tr>)}</tbody></table></div></section>
    <section className="panel purchase-summary"><div><span>Subtotal</span><strong>{formatPurchaseCurrency(sale.subtotal_ars, moneyPreferences)}</strong></div><div><span>Descuentos</span><strong>{formatPurchaseCurrency(sale.discount_total_ars, moneyPreferences)}</strong></div><div><span>Total</span><strong>{formatPurchaseCurrency(sale.total_ars, moneyPreferences)}</strong></div></section>
