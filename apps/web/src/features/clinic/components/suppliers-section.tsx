...
import type { Supplier, SupplierSummary } from "@/types/api";
...
export function SuppliersSection({ isExpanded, onToggle }: SuppliersSectionProps) {
  ...
  function replaceSupplier(supplier: SupplierSummary) {
  ...
  async function openEditSupplierForm(supplier: SupplierSummary) {
    setIsSaving(true);
    setMessage(null);
    try {
      const response = await getSupplier(supplier.id);
      const detail: Supplier = response.data;
      setFormState({
        id: detail.id,
        name: detail.name,
        ...
        <div className="settings-section-card__content">
              ...
              {sortedSuppliers.map((supplier) => (
                <article className="record-card clinic-team-card" key={supplier.id}>
                  <div>
                    ...
                    <p>
                      {[supplier.tax_id, supplier.phone, supplier.email]
                        .filter(Boolean)
          ...
          {formState ? (
            <form className="form-grid" onSubmit={handleSaveSupplier}>
              ...
              <label className="field">
                <span>Identificación fiscal / RUC (opcional)</span>
                <input
                  value={formState.tax_id}
                  onChange={(event) =>
                    setFormState((current) =>
                      current ? { ...current, tax_id: event.target.value } : current,
                    )
                  }
                />
              </label>
