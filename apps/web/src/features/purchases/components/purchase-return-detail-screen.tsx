...
export function PurchaseReturnDetailScreen({ returnId }: { returnId: string }) {
  ...
  return (
    <div className="page-stack purchases-page">
      ...
      <section className="inventory-table-card" aria-label="Productos devueltos"><div className="inventory-table-scroll"><table className="inventory-table purchase-table"><thead><tr><th>Producto</th><th>Cantidad</th><th>Unidad</th><th>Precio histórico</th><th>IVA</th><th>Subtotal</th><th>Total</th></tr></thead><tbody>{purchaseReturn.items.map((item) => <tr key={item.id} className="inventory-table__row--static"><td className="purchase-wrap"><strong>{item.description_snapshot}</strong><small>{item.internal_code_snapshot}</small></td><td>{item.quantity}</td><td>{item.unit}</td><td>{formatPurchaseCurrency(item.unit_price_without_tax_ars, moneyPreferences)}</td><td>{item.tax_rate_percentage}%</td><td>{formatPurchaseCurrency(item.line_subtotal_ars, moneyPreferences)}</td><td><strong>{formatPurchaseCurrency(item.line_total_ars, moneyPreferences)}</strong></td></tr>)}</tbody></table></div></section>
      <section className="panel purchase-summary"><div><span>Subtotal</span><strong>{formatPurchaseCurrency(purchaseReturn.subtotal_ars, moneyPreferences)}</strong></div><div><span>IVA</span><strong>{formatPurchaseCurrency(purchaseReturn.tax_total_ars, moneyPreferences)}</strong></div><div><span>Total devuelto</span><strong>{formatPurchaseCurrency(purchaseReturn.total_ars, moneyPreferences)}</strong></div></section>
