...
def test_currency_change_is_locked_after_monetary_activity(client, db_session, tenant):
    admin = _create_user(
        db_session, tenant, "currency-admin@example.com", "Clinic Admin", "clinic_admin"
    )
    ...
    response = client.patch(
        "/api/v1/clinic/preferences",
        headers=_user_headers(admin.email),
        json={"currency_code": "ARS", "locale": "es-AR"},
    )
